...
// TODO: stack of entity lists
// entitySaveState pushes a new copy
// entityLoadState pops a copy
static list_t entities[ENT_COUNT] = { { 0 } };
static list_t entitiestmp[ENT_COUNT] = { { 0 } };

static int uidFinder(const void* data, int* args) {
    entity_t* ent = (entity_t*)data;
    return (ent->uid == args[0]);
}

static int entityFinderPos(const void* data, int* args) {
    entity_t* ent = (entity_t*)data;
